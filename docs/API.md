...
- `provider?: 'openai' | 'anthropic' | 'google' | 'baseten'` - AI provider (default: 'openai')
- `tone?: 'neutral' | 'playful' | 'professional'` - Analysis tone (default: 'neutral')
- `model?: string` - AI model to use (defaults: `gpt-5.6-luna` at medium reasoning, `claude-sonnet-4-5`, `gemini-3-flash-preview`, or `BASETEN_MODEL` for Baseten)
...
- `provider: 'openai' | 'anthropic' | 'google' | 'baseten'` - AI provider (required)
...
- `provider?: 'openai' | 'anthropic' | 'google' | 'baseten'` - AI provider (required when `autoCensorProfanity` is set)
- `model?: string` - Model to use (defaults to the provider's chat model if omitted)
