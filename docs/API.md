...
- `model?: string` - AI model to use (defaults: `gpt-5.6-luna` at medium reasoning, `claude-sonnet-4-5`, `gemini-3-flash-preview`, `BASETEN_MODEL` for Baseten, or `OPENAI_COMPATIBLE_MODEL` for OpenAI-compatible)
...
- `model?: string` - Model to use (defaults: `text-embedding-3-small` for OpenAI, `gemini-embedding-001` for Google, `BASETEN_EMBEDDING_MODEL` for Baseten, or `OPENAI_COMPATIBLE_EMBEDDING_MODEL` for OpenAI-compatible)
