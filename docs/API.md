...
- `imageDownloadOptions?: object` - Options for image download when using base64 mode
  ...
  - `maxRetryDelay?: number` - Maximum delay between retries in milliseconds (default: 10000)
  - `exponentialBackoff?: boolean` - Whether to use exponential backoff (default: true)
  - `includeTranscript?: boolean` - When `true`, also moderate the caption transcript text for **video assets**, in addition to thumbnails (default: `false`). Only supported with provider `openai`; throws otherwise. Has no effect on audio-only assets, which always moderate transcript text. A valid transcript track is required on the asset when this option is enabled; if missing or malformatted, the workflow will report an error. A transcript that exists but has no parseable cues is different and is not a failure: it's reported as `skipped` in `transcriptModeration` (`skipReason: "no_cues"`) and the call still succeeds with thumbnails moderated. Transcription is never triggered. Transcript scores are returned in the dedicated `transcriptScores` array, separate from `thumbnailScores`; they never enter the thumbnail `coverage` denominator.
