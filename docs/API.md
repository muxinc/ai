...
- `imageDownloadOptions?: object` - Options for image download when using base64 mode
  ...
  - `maxRetryDelay?: number` - Maximum delay between retries in milliseconds (default: 10000)
  - `exponentialBackoff?: boolean` - Whether to use exponential backoff (default: true)
- `includeTranscript?: boolean` - When `true`, also moderate the caption transcript text for **video assets**, in addition to thumbnails (default: `false`). Only supported with provider `openai`; throws otherwise. Has no effect on audio-only assets, which always moderate transcript text. **A missing caption track or an empty transcript is a hard failure (throws)** — same contract as audio-only. A transcript that exists but has no parseable cues is different and not a failure: it's reported as `skipped` in `transcriptModeration` (`skipReason: "no_cues"`) and the call still succeeds with thumbnails moderated. Transcription is never triggered. Transcript scores are returned in the dedicated `transcriptScores` array, separate from `thumbnailScores`; they never enter the thumbnail `coverage` denominator.
- `transcriptWindowing?: object` - Optional tuning for transcript time-windowing (all fields optional; sensible defaults applied). Transcript moderation splits the caption track into **dynamic, overlapping** time windows whose size scales with the asset's duration: `windowSeconds = clamp(duration / targetWindowCount, minWindowSeconds, maxWindowSeconds)` and consecutive windows overlap by `max(minOverlapSeconds, windowSeconds * overlapFraction)`, so content straddling a window boundary is still scored intact.
  ...
  - `maxWindowSeconds?: number` - Upper clamp on window size, in seconds (default: `120`)
...
```typescript
{
  ...
  transcriptModeration: { // Audit trail for transcript moderation: was it requested, did it complete, and if skipped, why?
    ...
    // 'skipped'   — requested but no windows were moderated (see `skipReason` / `skipMessage`). A missing
    //                track or empty transcript throws instead of reaching this state (see `includeTranscript` above).
    ...
    // A missing caption track or an empty transcript throws instead of skipping (see `includeTranscript` above).
    skipReason?: 'no_cues';
...
**Transcript moderation is auditable, not silent — and a missing/empty transcript is a hard failure, not a silent skip.** Whether the caller passed `includeTranscript: true` on a video asset or the asset was audio-only (implicit request), a missing caption track or an empty transcript **throws** rather than completing with degraded results — callers who ask for transcript moderation get an error if there was nothing to moderate, the same way audio-only assets always have. The one exception is a transcript that fetches successfully but has no parseable cues: that's a legitimate "moderated, nothing to find" outcome, not a failure, and is reported via `transcriptModeration: TranscriptModerationStatus` as `skipped` (`skipReason: "no_cues"`) alongside a human-readable `skipMessage`, with the call still succeeding.
