...
For audio-only assets, moderation runs on the transcript; the thumbnail surface is reported as skipped (`thumbnailModeration.skipReason: "no_video_track"`). Use OpenAI, the only provider that supports text moderation. A ready caption track is required — with nothing to moderate, the call throws.
