...
const EnvSchema = v.pipe(
  v.objectWithRest({
    NODE_ENV: optionalString("Runtime environment."),
...
export type Env = v.InferOutput<typeof EnvSchema> & { NODE_ENV: string };
...
function parseEnv(): Env {
  ...
  if (!parsedEnv.success) {
    ...
    console.error(JSON.stringify(v.flatten(parsedEnv.issues).nested ?? {}, null, 2));
    process.exit(1);
  }

  return {
    ...parsedEnv.output,
    NODE_ENV: parsedEnv.output.NODE_ENV ?? "development",
  };
}
