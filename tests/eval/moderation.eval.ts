 ...
 */
const LATENCY_THRESHOLD_GOOD_MS = 15000;

/**
 * Maximum acceptable latency in milliseconds for "acceptable" performance.
 */
const LATENCY_THRESHOLD_ACCEPTABLE_MS = 45000;

/**
 * Ceiling for safe-content scores. Safe assets should score below this value
 * for both sexual and violence categories, leaving a comfortable margin below
 * the default workflow thresholds (sexual: 0.7, violence: 0.8).
 */
const SAFE_SCORE_CEILING = 0.5;

/**
 * Floor for violence scores when the asset is known to contain violent content.
 * The violent test asset should produce a violence score above this value.
 */
const VIOLENT_SCORE_FLOOR = 0.3;
...
evalite("Moderation", {
  ...
  scorers: [
    ...
    // SCORE CALIBRATION (SAFE): For safe assets, both scores should remain low
    {
      ...
      description: `For safe content, validates that both sexual and violence maxScores are below ${SAFE_SCORE_CEILING}. Skips for expected-flagged content.`,
      scorer: ({ output, expected }: { output: EvalOutput; expected: Expected }) => {
        ...
        const sexualOk = output.maxScores.sexual < SAFE_SCORE_CEILING;
        const violenceOk = output.maxScores.violence < SAFE_SCORE_CEILING;
