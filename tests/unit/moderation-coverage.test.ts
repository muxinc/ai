...
describe("getModerationScores coverage metadata", () => {
  ...
  it("downloads with backoff and resubmits when OpenAI cannot fetch a thumbnail URL", async () => {
    const thumbnailUrl = "https://image.mux.com/playback-123/thumbnail.png?time=0&width=640";
    vi.mocked(getThumbnailUrls).mockResolvedValue([{ url: thumbnailUrl, time: 0 }]);
    let thumbnailFetches = 0;

    mockFetch.mockImplementation(async (url, init) => {
      if (url === thumbnailUrl) {
        thumbnailFetches += 1;
        return thumbnailFetches === 1 ?
      ...
      const body = JSON.parse(String(init?.body));
      const submittedImage = body.input[0].image_url.url as string;
      if (submittedImage === thumbnailUrl) {
        return mockOpenAIModerationResponse({
          status: 400,
          body: {
            error: {
              message: `Could not download file from URL provided: ${thumbnailUrl}.`,
      ...
      return mockOpenAIModerationResponse({
        status: 200,
        body: { results: [{ category_scores: { sexual: 0.1, violence: 0.2 } }] },
      });
    });
    ...
    expect(thumbnailFetches).toBe(2);
    expect(result.coverage.successfulSampleCount).toBe(1);
    expect(result.thumbnailScores[0]?.error).toBe(false);
  });

  it("keeps base64 download failures isolated to their thumbnail", async () => {
    const missingUrl = "https://image.mux.com/playback-123/thumbnail.png?time=0&width=640";
    const readyUrl = "https://image.mux.com/playback-123/thumbnail.png?time=10&width=640";
    vi.mocked(getThumbnailUrls).mockResolvedValue([
      { url: missingUrl, time: 0 },
      { url: readyUrl, time: 10 },
    ]);
    ...
    const result = await getModerationScores("asset-123", {
      ...
      imageDownloadOptions: { retries: 0 },
    });
